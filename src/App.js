...
function LoginScreen({ onLoginSuccess, auth }) {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            // onLoginSuccess is handled by onAuthStateChanged in App component
        } catch (error) {
...
function App() {
    ...
    // Initial loading screen before auth is ready
    if (!isAuthReady) {
        return (
...
const MarkCompleteModal = ({ isOpen, onClose, onConfirm, order }) => {
    ...
    const handleConfirm = () => {
        onConfirm(order.id, completionDate);
    };
    ...
    const handleFormSubmit = async (filmData) => {
        ...
        try {
            if (editingFilm) {
                ...
                await updateDoc(filmRef, filmData);
                setEditingFilm(null);
            } else {
                await addDoc(collection(db, filmsCollectionPath), { ...filmData, currentWeight: filmData.netWeight, createdAt: new Date() });
            }
            setShowForm(false);
...
function FilmForm({ onSubmit, onCancel, initialData }) {
    const [formData, setFormData] = useState({ filmType: '', netWeight: '', supplier: '', purchaseDate: new Date().toISOString().split('T')[0] });

    useEffect(() => {
        const defaultState = { filmType: '', netWeight: '', supplier: '', purchaseDate: new Date().toISOString().split('T')[0] };
        if (initialData) {
            setFormData({
                filmType: initialData.filmType || '', netWeight: initialData.netWeight || '',
                supplier: initialData.supplier || '',
                purchaseDate: initialData.purchaseDate ? new Date(initialData.purchaseDate).toISOString().split('T')[0] : defaultState.purchaseDate,
            });
                ...
                <tbody>
                    {films.map(film => (
                        <tr key={film.id} className="border-b border-gray-700 hover:bg-gray-700/50">
                            ...
                            <td className="p-3">{film.supplier}</td><td className="p-3">{new Date(film.purchaseDate).toLocaleDateString()}</td>
...
function JobManagement({ films, jobs, orders, db, userId }) {
    ...
    const handleJobSubmit = async (jobData) => {
        if (!db || !userId) return;
        try {
            await addDoc(collection(db, `artifacts/${appId}/users/${userId}/jobs`), { ...jobData, createdAt: new Date() });
            setShowForm(false);
    ...
    return (
        <section>
            ...
            {showForm && <JobForm films={films} onSubmit={handleJobSubmit} onCancel={() => setShowForm(false)} />}
            <JobList films={films} jobs={filteredJobs} onDelete={openDeleteModal} db={db} userId={userId} />
            <ConfirmationModal
                isOpen={isDeleteModalOpen}
                onClose={closeDeleteModal}
...
function JobForm({ onSubmit, onCancel, films }) {
    ...
    return (
        <div className="bg-gray-800 p-6 rounded-lg mb-8 shadow-lg">
            <h3 className="text-xl font-semibold mb-4 text-cyan-400">Add New Job</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-center space-x-4 pt-2">
                    <button type="submit" className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg w-full">Create Job</button>
...
function JobList({ films, jobs, onDelete, db, userId }) {
    if (jobs.length === 0) return <p className="text-center text-gray-500 py-8">No jobs found.</p>;
    return <div className="space-y-4">{jobs.map(job => <JobCard key={job.id} job={job} films={films} onDelete={onDelete} db={db} userId={userId} />)}</div>;
}

function JobCard({ job, films, onDelete, db, userId }) {
    ...
    return (
        <>
            <div className="bg-gray-800 rounded-lg p-4 shadow-md border-l-4 border-gray-600">
                ...
                <div className="mt-4">
                    <h4 className="font-semibold text-sm text-gray-300 mb-1">Material Memo:</h4>
                    <ul className="list-disc list-inside text-gray-400 text-sm space-y-1">
                        {job.materials?.length > 0 ? job.materials.map((mat, index) => (<li key={index}>{mat}</li>)) : <li>No materials specified</li>}
                    </ul>
                </div>
                        ...
                        {isLoadingHistory ? <p>Loading history...</p> : (
                            history.length > 0 ? (
                                <ul className="space-y-2">
                                    {history.map(roll => (
                                        <li key={roll.id} onClick={() => setViewingRoll(roll)} className="p-2 bg-gray-700 rounded-md cursor-pointer hover:bg-gray-600">
                                            <p className="font-semibold">{roll.filmType}</p>
                                            <p className="text-sm text-gray-400">Consumed on: {roll.consumedAt?.toDate().toLocaleString()}</p>
                                        </li>
                                    ))}
                                </ul>
            ...
            {viewingRoll && <RollDetailModal roll={viewingRoll} onClose={() => setViewingRoll(null)} />}
        </>
    );
}

function RollDetailModal({ roll, onClose }) {
    return(
        <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50 p-4">
            <div className="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md m-4">
                <div className="flex justify-between items-center mb-4">
                     <h3 className="text-2xl font-bold text-cyan-400">Consumed Roll Details</h3>
                     <button onClick={onClose} className="text-gray-400 hover:text-white"><XIcon/></button>
                </div>
                <div className="space-y-3 text-gray-300">
                    <p><strong className="text-gray-100 w-32 inline-block">Film Type:</strong> {roll.filmType}</p>
                    <p><strong className="text-gray-100 w-32 inline-block">Original Weight:</strong> {roll.netWeight?.toFixed(2)} kg</p>
                    <p><strong className="text-gray-100 w-32 inline-block">Supplier:</strong> {roll.supplier}</p>
                    <p><strong className="text-gray-100 w-32 inline-block">Purchase Date:</strong> {new Date(roll.purchaseDate).toLocaleDateString()}</p>
                    <p><strong className="text-gray-100 w-32 inline-block">Consumed Date:</strong> {roll.consumedAt?.toDate().toLocaleString()}</p>
                    <p><strong className="text-gray-100 w-32 inline-block">Original ID:</strong> <span className="text-xs font-mono">{roll.originalId || roll.id}</span></p>
                </div>
                 <button onClick={onClose} className="mt-6 w-full bg-cyan-600 hover:bg-cyan-700 text-white font-bold py-2 px-4 rounded-lg">Close</button>
            </div>
        </div>
    )
}
            ...
            <div className="flex justify-between items-start gap-4">
                <div>
                    ...
                    <div className="flex flex-wrap gap-x-4 text-sm text-gray-300 mt-1">
                     ...
                     <p className="text-xs text-gray-500 mt-1">Ordered: {order.createdAt?.toDate().toLocaleString()}</p>
                     {order.status === 'completed' && <p className="text-xs text-purple-400 mt-1">Completed: {order.completedAt?.toDate().toLocaleDateString()}</p>}
                </div>
                                    ...
                                    <ul className="space-y-2 text-sm">
                                        {history.map(roll => (
                                            <li key={roll.id} className="p-2 bg-gray-700/50 rounded-md">
                                                ...
                                                <p className="text-gray-400">Consumed: {roll.consumedAt?.toDate().toLocaleString()}</p>
                                            </li>
                                        ))}
                                    </ul>
...
function UseStock({ films, jobs, db, userId, setView }) {
    ...
    const [dateOfUse, setDateOfUse] = useState(new Date().toISOString().split('T')[0]);
    const jobSearchRef = useRef(null);
    const [messageModal, setMessageModal] = useState({isOpen: false, title: '', body: ''});
    ...
    const handleUseRoll = async () => {
        ...
        const consumedData = {
            ...
            supplier: selectedRoll.supplier || "N/A", purchaseDate: selectedRoll.purchaseDate || new Date().toISOString().split('T')[0],
            createdAt: selectedRoll.createdAt, originalId: selectedRoll.id, consumedAt: new Date(`${dateOfUse}T00:00:00`),
            ...
            try {
                ...
                allHistorySnapshots.forEach(snapshot => {
                    snapshot.docs.forEach(doc => {
                        combinedHistory.push({ id: doc.id, ...doc.data() });
                    });
                });
                ...
                <div className="space-y-3">
                    {filteredHistory.length > 0 ? filteredHistory.map(item => (
                        <div key={item.id} className="bg-gray-800 p-4 rounded-lg">
                            ...
                            <p className="text-gray-400 text-sm">Date Used: {item.consumedAt.toDate().toLocaleString()}</p>
